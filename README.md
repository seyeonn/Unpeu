...
~~[앙뿌 사이트 바로가기](http://unpeu.co.kr/landing)~~ (현재 지원하지 않습니다.)

[서비스 이용가이드 바로가기](https://www.notion.so/unpeu-27a3d8503afa40a3829c6ba4df81d4a3)
...
1. 축하 메세지와 선물을 같이 보낼 수 없는 불편함 (일부 서비스는 가능)
...
|Jira| Agile 방식으로 프로젝트 관리하기 위한 협업 툴 | |
|Google Docs| 프로젝트 산출물 관리 | |
|GitLab| 소스 코드를 효과적으로 관리하는 형상 관리 도구(버전 관리 시스템) | |
...
- 황보라
    1. API(Google Docs), DB 설계(ERD CLOUD)
    2. FrontEnd(Vue.js)
        1. 랜딩페이지 구현
        2. 소셜로그인 구현
        3. 유저 프로필 구현
    3. Backend(SpringBoot)
        1. 유저 관리 REST API 구현
        2. 카카오와 구글 소셜로그인 REST API 구현
        3. Spring Security 를 활용해서 인증/인가 구현
        4. JPA활용 Database관리 
    4. 발표 
- 김정윤
    1. API(Google Docs), DB 설계(ERD CLOUD)
    2. FrontEnd (Vue.js)
        1. 다이어리 게시글, 댓글 레이아웃 및 기능 구현
        2. Editor를 활용한 게시글 작성
    3. Backend (SpringBoot)
        1. 다이어리 관리 REST API 구현 (게시글, 댓글 CRUD)
        2. 메세지 다이어리로 저장
        3. 다이어리 TDD
...
- 손모은
    1. API(Google Docs), DB 설계(ERD CLOUD)
    2. FrontEnd(Vue.js) 
        1. 선물 결제 레이아웃 및 기능 구현(메세지 보내기, 선물 결제)
        2. 메인 페이지 컨셉 변경 기능 구현(default/children/birthday 변경 기능)
        3. Feedback 레이아웃 및 기능 구현
    3. BackEnd(SpringBoot)
